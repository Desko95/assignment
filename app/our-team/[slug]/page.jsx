...
async function fetchTeamMember(slug) {
    ...
    const MembersPromise = await fetch(`${process.env.BE_HOST}/api/team-members?${ourQuery}`)
    const member = await MembersPromise.json()
    return member.data[0]  //can be a .content instead of a 0
}
    ...
    return (
        <div>
            <div className="text-white relative bg-gray-700 px-14 py-16 -mx-8 -mt-7">
                ...
                <img className="object-cover absolute top-0 bottom-0 left-1/2 right-0 block w-1/2 h-full opacity-50 fitler grayscale"
                     src={`${process.env.BE_HOST}${member.photo.formats.medium.url}`} alt={member.name}
                />
                <div className="absolute z-20 w-80 bg-gradient-to-r from-gray-700 to-transparent h-full top-0 bottom-0 left-1/2"></div>
            </div>

            <div className="transform -translate-y-1/2">
                <Link href="/our-team" className="text-sm bg-gray-600 text-gray-400 hover:bg-gray-500 hover:text-gray-300 inline-block rounded-lg py-3 px-5" >&laquo; Back to all team members</Link>
            </div>
